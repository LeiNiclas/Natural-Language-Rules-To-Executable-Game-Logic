...
% Flat list representation for the 3x3 board.
...
% CurrentPlayer = atom (x | o)

initial_state(state([empty, empty, empty, empty, empty, empty, empty, empty, empty], x)).

current_player(state(_, P), P).

legal_move(state(Board, Player), Pos) :-
    integer(Pos),
    Pos >= 1,
    Pos =< 9,
    nth1(Pos, Board, empty).

apply_move(state(Board, CurrentPlayer), Pos, state(NewBoard, NextPlayer)) :-
    integer(Pos),
    Pos >= 1,
    Pos =< 9,
    nth1(Pos, Board, empty),
    set_nth1(Pos, Board, CurrentPlayer, NewBoard),
