...
check_diagonal_down(Board, Player) :-
    ...
    RowIdx >= 4,
    ColIdx =< 4,
    PrevRow1 is RowIdx - 1,
    PrevRow2 is RowIdx - 2,
    PrevRow3 is RowIdx - 3,
    NextCol1 is ColIdx + 1,
    NextCol2 is ColIdx + 2,
    NextCol3 is ColIdx + 3,
    nth1(PrevRow1, Board, Row1),
    nth1(PrevRow2, Board, Row2),
    nth1(PrevRow3, Board, Row3),
    nth1(NextCol1, Row1, Player),
    nth1(NextCol2, Row2, Player),
    nth1(NextCol3, Row3, Player).
...
render_row(Row) :-
    maplist(render_cell, Row, CharList),
    atomic_list_concat(CharList, ' ', Line),
    format('~s~n', [Line]).
